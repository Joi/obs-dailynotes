...
async function generateDashboard(reminders) {
  ...
  if (presentations) {
    try {
      ...
      if (presList.length > 0) {
        ...
        markdown += `### 🎤 Presentations (${presStats.todo} active)\n\n`;
        ...
        const inProgress = presList.filter(p => p.status === 'todo' && p.startedDate && !urgent.includes(p));
        const planned = presList.filter(p => p.status === 'todo' && !p.startedDate && !urgent.includes(p));

        const MAX_PRESENTATIONS = 10;
        let shown = 0;

        // Helper function to format presentation with all links
        const formatPresentation = (p, icon) => {
          const dueStr = p.deadline ? ` (due ${p.deadline})` : '';
          let line = `- ${icon} [${p.title}](${p.url})${dueStr}`;
          const links = [];
          if (p.notionUrl) links.push(`[📝 Notion](${p.notionUrl})`);
          if (p.slackUrl) links.push(`[💬 Slack](${p.slackUrl})`);
          if (links.length > 0) {
            line += ` | ${links.join(' | ')}`;
          }
          return line + '\n';
  ...
  // 6. AMPLIFIER PROJECTS (software development initiatives)
  const switchboardPath = process.env.SWITCHBOARD_DATA_PATH
    ? path.dirname(process.env.SWITCHBOARD_DATA_PATH)
    : path.join(require('os').homedir(), 'switchboard');
  const amplifierSection = await buildAmplifierProjectsSection(switchboardPath);
  if (amplifierSection) {
    markdown += amplifierSection + '\n';
  }

  // 7. ACTIVE TASKS BY PROJECT (reminder tags, stats only)
  const projects = getProjectStats(reminders);
  if (Object.keys(projects).length > 0) {
    markdown += `## 📌 Active Tasks by Project\n\n`;
  ...
  // 8. SOMEDAY (show count, hide details to reduce noise)
