...
/**
 * Parse Microsoft Teams meetings
 */
function parseTeams(event) {
    ...
    const findTeams = (txt) => findUrlWithHint(txt, 'teams.microsoft.com') || findUrlWithHint(txt, 'teams.live.com');
    // Structured conferenceData (preferred when present)
    if (event.conferenceData && Array.isArray(event.conferenceData.entryPoints)) {
        for (const ep of event.conferenceData.entryPoints) {
            const uri = ep.uri || ep.label || '';
            const hit = findTeams(uri);
            if (hit) { output.url = hit; break; }
        }
    }
    if (!output.url && event.location && event.location.indexOf('http') != -1)
        output.url = findTeams(event.location);
    if (!output.url && event.description && event.description.indexOf('http') != -1)
        output.url = findTeams(event.description);
    if (!output.url && event.summary && event.summary.indexOf('http') != -1)
        output.url = findTeams(event.summary);
...
/**
 * Parse Cisco Webex meetings
 */
function parseWebex(event) {
    let output = {
        'callTitle': '', 'fullStartDate':'', 'callStart': '', 'callEnd': '',
        'attendees': [], 'url': '', 'type': 'videoCall', 'location': ''
    };
    ...
    const findWebex = (txt) => findUrlWithHint(txt, 'webex.com') || findUrlWithHint(txt, 'cisco.com');
    // Structured conferenceData (preferred when present)
    if (event.conferenceData && Array.isArray(event.conferenceData.entryPoints)) {
        for (const ep of event.conferenceData.entryPoints) {
            const uri = ep.uri || ep.label || '';
            const hit = findWebex(uri);
            if (hit) { output.url = hit; break; }
        }
    }
    if (!output.url && event.location && event.location.indexOf('http') != -1)
        output.url = findWebex(event.location);
    if (!output.url && event.description && event.description.indexOf('http') != -1)
        output.url = findWebex(event.description);
    if (!output.url && event.summary && event.summary.indexOf('http') != -1)
        output.url = findWebex(event.summary);
