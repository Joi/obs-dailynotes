...
function buildOwnerVariantsFromEnv() {
  ...
  if (variants.size === 0) {
    ['<Owner Name>', '<Owner Name>', '<Owner Name> Mailstore'].forEach(add);
  }
  return variants;
}
  ...
  try {
    ...
    if (emailsList) {
      ...
      if (first) {
        ...
        console.warn(`  SKIP_PREFETCH=1 PERSON_FILE="/Users/<Owner>/switchboard/${personKey}.md" node tools/enrichFromLLM.js`);
      }
    }
  } catch {}
}
