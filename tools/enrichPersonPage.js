...
if (args.length === 0) {
  ...
  console.log('  --shared         Create as a shared list (Joi/<Name> To Do)');
...
const fullPath = path.isAbsolute(filePath) 
  ? filePath 
  : path.join('/Users/joi/switchboard', filePath);
...
if (shouldCreateList) {
  ...
  if (createShared) {
    ...
    fm.reminders.sharedListName = `Joi/${personName.split(' ')[0]} To Do`;
...
if (shouldCreateList && createShared) {
  const listName = `Joi/${personName.split(' ')[0]} To Do`;
